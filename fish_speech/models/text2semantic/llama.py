...
def _construct(model_cls, config: BaseModelArgs, path: str, init_weights: bool):
    """Build the module and apply any quantization the path asks for."""
    model = model_cls(config, init_weights=init_weights)
    ...
    if "int4" in str(Path(path)):
        ...
        model = WeightOnlyInt4QuantHandler(model, groupsize).convert_for_runtime()
...
class BaseTransformer(nn.Module):
    ...
    def from_pretrained(
        ...
        if load_weights is False:
            ...
            # billion parameters costs minutes, and a checkpoint that covers
            # the whole model overwrites every one of them.
            with torch.device("meta"):
                model = _construct(model_cls, config, path, init_weights=False)

            weights = _load_checkpoint_weights(Path(path), device, dtype)
            err = model.load_state_dict(weights, strict=False, assign=True)

            if err.missing_keys:
                # Loading is not strict, so a checkpoint covering part of the
                # model has always been allowed and finetuning relies on it.
                # Those weights do need random values, so build again.
                logger.warning(
                    f"{path} has no weights for {err.missing_keys}; "
                    "initializing them randomly"
                )
                model = _construct(model_cls, config, path, init_weights=True)
