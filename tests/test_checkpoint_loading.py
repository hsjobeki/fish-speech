...
class TestCheckpointLoading(unittest.TestCase):
    ...
    def test_incomplete_checkpoint_raises(self):
        """Without random init a missing key leaves an unusable meta
        tensor, so the load has to fail instead of returning it."""
        with tempfile.TemporaryDirectory() as tmp:
            path = write_checkpoint(Path(tmp), drop="layers.0.attention.wo.weight")

            with self.assertRaises(RuntimeError) as caught:
                BaseTransformer.from_pretrained(str(path), load_weights=True)

        self.assertIn("layers.0.attention.wo.weight", str(caught.exception))
